...
export const dataImportRouter = createTRPCRouter({
  ...
  uploadFile: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      console.log(`User ${ctx.session.user.id} is uploading file: ${input.fileName}`);
  ...
  getRecords: protectedProcedure
    .query(async ({ ctx }) => {
      ...
      // const { data, error } = await ctx.supabase.from('import_records').select('*').eq('user_id', ctx.session.user.id);
  ...
  getRecordDetails: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // const { data, error } = await ctx.supabase.from('import_records').select('*').eq('id', input.recordId).eq('user_id', ctx.session.user.id).single();
