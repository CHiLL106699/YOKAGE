...
// LemonSqueezy 訂閱方案
export const lemonsqueezyPlans = mysqlTable('lemonsqueezy_plans', {
  ...
  interval: mysqlEnum('interval', ['month', 'year', 'one_time']).notNull(), // 計費週期
  intervalCount: int('interval_count').notNull().default(1), // 計費週期數量
  isActive: int('is_active').notNull().default(1), // 是否啟用
  createdAt: timestamp('created_at').notNull().defaultNow(),
  updatedAt: timestamp('updated_at').notNull().defaultNow().onUpdateNow(),
});

// LemonSqueezy 訂閱記錄
export const lemonsqueezySubscriptions = mysqlTable('lemonsqueezy_subscriptions', {
  id: int('id').primaryKey().autoincrement(),
  organizationId: int('organization_id').notNull(), // 診所 ID
  userId: int('user_id').notNull(), // 用戶 ID
  planId: int('plan_id').notNull(), // 方案 ID
  ...
  status: mysqlEnum('status', ['active', 'cancelled', 'expired', 'on_trial', 'paused', 'past_due', 'unpaid']).notNull(), // 訂閱狀態
...
// LemonSqueezy 付款記錄
export const lemonsqueezyPayments = mysqlTable('lemonsqueezy_payments', {
  id: int('id').primaryKey().autoincrement(),
  organizationId: int('organization_id').notNull(), // 診所 ID
  userId: int('user_id').notNull(), // 用戶 ID
  subscriptionId: int('subscription_id'), // 訂閱 ID（若為訂閱付款）
  ...
  currency: varchar('currency', { length: 10 }).notNull().default('TWD'), // 幣別
  status: mysqlEnum('status', ['pending', 'paid', 'failed', 'refunded']).notNull(), // 付款狀態
...
export const lemonsqueezyWebhookEvents = mysqlTable('lemonsqueezy_webhook_events', {
  id: int('id').primaryKey().autoincrement(),
  ...
  processed: int('processed').notNull().default(0), // 是否已處理
